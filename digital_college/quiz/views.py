...
from .forms import quiz_detail_form, single_correct_form, multi_correct_form, truefalse_form, response_single_form, \
    response_multiple_form, response_true_form, answer_form
from .models import quiz as qz, singlechoice, multiplechoice, truefalse, answers, respo_single, respo_multiple, \
    respo_true, result, test_given
...
def take_quiz(request, class_name, quiz_name):
    ...
    if (test_given.objects.filter(student_id=reg_user_instance, quiz_id=quiz_instance).exists()):
        ...
        return render(request, 'quiz/quiz_list.html',
                      {'quizzes': quizzes, 'class_name': class_name, 'message': 'You cannot take this quiz again!'})
    else:
        question_single = singlechoice.objects.filter(college_id=college_instance, class_id=course_instance,
                                                      quiz_id=quiz_instance)
        question_multiple = multiplechoice.objects.filter(college_id=college_instance, class_id=course_instance,
                                                          quiz_id=quiz_instance)
        question_true = truefalse.objects.filter(college_id=college_instance, class_id=course_instance,
                                                 quiz_id=quiz_instance)
        ...
        if request.method == "POST":
            ...
            if response_single_sets.is_valid() and response_multiple_sets.is_valid() and response_true_sets.is_valid():
                ...
                return redirect('after:classroom:quiz:quiz_result', quiz_name=quiz_instance.name_of_quiz,
                                class_name=course_instance)
        else:
    ...
    for res in all_multi_responses:
        ...
        if res.selected_option:
            ...
            if len(answer_instances) == len(option_list):
                ...
                pass
        total_marks = total_marks + res.question_id.marks
    result_instance = result(quiz_id=quiz_instance, student_id=student_instance, marks_obtained=marks,
                             total_marks=total_marks)
    result_instance.save()
    return render(request, 'quiz/quiz_result.html',
                  {'marks': marks, 'total_marks': total_marks, 'class_name': class_name})
